...
def aggregate_cfb(fd_lines, dk_lines, pb_lines, fb_lines, wh_lines, r_lines, bet365_lines):
        #do this for each webstie and create 5 dicts, fd_key:site_key
    bet365_lines = {}
    fd_to_pb = {}
    fd_to_dk = {}
    fd_to_resorts = {}
    ...
    fd_to_wh = {}
    for date in fd_lines.keys():
        for key in fd_lines[date].keys():
            if date in dk_lines.keys():
                k0 = process.extractOne(key, dk_lines[date].keys(), scorer=fuzz.token_set_ratio)
                if(k0[1] > 90):
                    fd_to_dk[key] = k0[0]
            if date in r_lines.keys():        
                k = process.extractOne(key, r_lines[date].keys(), scorer=fuzz.token_set_ratio)
                if k[1] >= 80:
                    fd_to_resorts[key] = k[0]
            if date in fb_lines.keys():
                k1 = process.extractOne(key, fb_lines[date].keys(), scorer=fuzz.token_set_ratio)
                if k1[1] >= 80:
                    fd_to_fb[key] = k1[0]
            if date in pb_lines.keys():
                k2 = process.extractOne(key, pb_lines[date].keys(), scorer=fuzz.token_set_ratio)
                if k2[1] >= 80:
                    fd_to_pb[key] = k2[0]
            if date in wh_lines.keys():
                k3 = process.extractOne(key, wh_lines[date].keys(), scorer=fuzz.token_set_ratio)
                if k3[1] >= 80:
                    fd_to_wh[key] = k3[0]
    ...
    for date in fd_lines.keys():
        ...
        for fd_team in fd_lines[date].keys():
            ...
            if(date in pb_lines.keys()):
                try:
                    pb_val = pb_lines[date][fd_to_pb[fd_team]]
                    #print('PB val: ', pb_val)
                except:
                    pb_val = 0
            else:
            ...
            if(date in fb_lines.keys()):
                try:
                    fb_val = fb_lines[date][fd_to_fb[fd_team]]
                    #print("FB val: ", fb_val)
                except:
                    #print('Field not in FB')
                    fb_val = 0
            ...
            if(date in wh_lines.keys()):
                try:
                    wh_val = wh_lines[date][fd_to_wh[fd_team]]
                except: 
                    wh_val = 0
            else:
                wh_val = 0
            if(date in r_lines.keys()):
                try:
                    r_val = r_lines[date][fd_to_resorts[fd_team]]
                except Exception as e: 
                    print(e)
                    r_val = 0
            else:
